...
The goal here is to have a good default query implementation, and make it possible to plug in your own.
...
It will support:
  ...
  - Conversion from externally mapped object paths to internal ones.
  - Generation of Expresion trees for predicates.
  - Short hand for filter=(some.property$eq$something) is some.property=something

The filter will be specified in HTTP query parameter q.

It uses character '$' as separation char, although this might change.
...
These operators are candidates for being supported:

* eq: equals..
* gt,gte,lt,lte: Greater than, greater than or equal, less than, less than or equal
* like: Similar to SQL LIKE operator
* in: Takes a range of values, for example q=bar$in$2,3,4,5
